	...
	local function export_h264_to_file()
		...
		local function real_write_to_file(stream_info, str_bytes, begin_with_nalu_hdr)
			if first_run then
				...
				if stream_info.counter2 == 0 or stream_info.counter2 == 1 then
				...
				-- smallgoby
				local strLen=string.len(str_bytes)
				local strPrint=""
				local tmpI = 1
				repeat
					local tmp = string.format("%2X",string.byte(str_bytes,tmpI))
					
					if (string.sub(tmp, 1, 1) == " ") then
						tmp = string.sub(tmp,2,2)
						tmp="0" .. tmp
					end

					strPrint=strPrint .. tmp
					tmpI=tmpI+1
				until tmpI>strLen
				print(strPrint)
				
				
				if begin_with_nalu_hdr and stream_info.counter2 > 1 then
					-- *.264 raw file format seams that every nalu start with 0x00000001\
					-- smallgoby
					--if flag then
					--	stream_info.file:write("\00\00\01")
					--	twappend("byte:\00\00\01" )
					--else
					--	stream_info.file:write("\00\00\00\01")
					-- end
					local ii,jj,iii,jjj
					ii=0
					iii=0
					kk,jj=string.find(strPrint, "41") -- 419A 419B
					kkk,jj=string.find(strPrint, "67640015ACB201004B6022")
					kkkk,jj=string.find(strPrint, "68EBC3CB22C0")
					 --ii,jj=string.find(strPrint, "419A3B117F71B09421")
					 -- iii,jjj=string.find(strPrint, "419A4F0864CA6117FF684A2AA8")--419A4F0864CA6117FF684A2AA8
					 -- iiii,jjj=string.find(strPrint, "419A727843C994C08BFFC6BB")
					-- 04 00 3d 00 d7 00  
					-- 41 9a 92 78 43 c9 94 c0 8b ff fa 58 4a ff 09
					if kk==1  or kkk==1 or kkkk==1 then -- or ii==1 or iii==1 or iiii==1
						stream_info.file:write("\00\00\00\01")
					else
						stream_info.file:write("\00\00\01")
					end
		...
		local function sort_and_write(stream_info, frame)
			...
			if completed then
				for i = 1, #frame.packs, 1 do
					real_write_to_file(stream_info, frame.packs[i].data, frame.packs[i].nalu_begin,false)
				end
			else
		...
		function my_h264_tap.packet(pinfo,tvb)
			...
			if not pinfo.dst_port == 6970 then
				return
			end
			
			if not pinfo.src_port == 62447 then
				return
			end
			
			
			if tostring(pinfo.dst) == "192.168.1.100" then
				return
			end
		...
		local function export_h264(drop_frame)
			...
			-- retap_packets() --smallgoby cancel
			first_run = false
